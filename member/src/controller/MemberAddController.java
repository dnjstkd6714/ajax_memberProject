// 2018. 8. 8(수) 이원상	MemberAddController.java
package controller;
...
public class MemberAddController extends HttpServlet {
    ...
    public MemberAddController() {					// default생성자
        super();									// 조상클래스 생성자 호출
    }
    //dopost 메소드 오버라이딩
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean addResult = this.memberService.addMember(this.member);	// 회원등록 메소드 호출
																		// 입력된 행이 있을경우 true, 없을시 false
		if(addResult) {	
